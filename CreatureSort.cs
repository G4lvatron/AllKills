namespace AllKills
{
    ...
    internal class CreatureSort
    {
        ...
        /// <returns> Number representing the order. </returns>
        public static int GetCreatureValue(IconSymbol.IconSymbolData creature)
        {
            ...
            if (type == MoreSlugcats.MoreSlugcatsEnums.CreatureTemplateType.SpitLizard)     return 10;
            if (type == MoreSlugcats.MoreSlugcatsEnums.CreatureTemplateType.ZoopLizard)     return 11;
            ...
            if (type == MoreSlugcats.MoreSlugcatsEnums.CreatureTemplateType.EelLizard)      return 16;
            ...
            if (type == MoreSlugcats.MoreSlugcatsEnums.CreatureTemplateType.JungleLeech)    return 23;
            ...
            if (type == MoreSlugcats.MoreSlugcatsEnums.CreatureTemplateType.ScavengerElite) return 27;
            ...
            if (type == CreatureTemplate.Type.Centipede)                                    return 33 + RWCustom.Custom.IntClamp(creature.intData, 1, 3) - 1;
            if (type == CreatureTemplate.Type.RedCentipede)                                 return 36;
            if (type == CreatureTemplate.Type.Centiwing)                                    return 37;
            if (type == MoreSlugcats.MoreSlugcatsEnums.CreatureTemplateType.AquaCenti)      return 38;
            ...
            if (type == MoreSlugcats.MoreSlugcatsEnums.CreatureTemplateType.MotherSpider)   return 44;
            if (type == CreatureTemplate.Type.SpitterSpider)                                return 45;
            if (type == CreatureTemplate.Type.MirosBird)                                    return 46;
            if (type == MoreSlugcats.MoreSlugcatsEnums.CreatureTemplateType.MirosVulture)   return 47;
            if (type == CreatureTemplate.Type.BrotherLongLegs)                              return 48;
            if (type == CreatureTemplate.Type.DaddyLongLegs)                                return 49;
            if (type == MoreSlugcats.MoreSlugcatsEnums.CreatureTemplateType.TerrorLongLegs) return 50;
            if (type == MoreSlugcats.MoreSlugcatsEnums.CreatureTemplateType.Inspector)      return 51;
            ...
            if (type == MoreSlugcats.MoreSlugcatsEnums.CreatureTemplateType.StowawayBug)    return 56;
            ...
            if (type == MoreSlugcats.MoreSlugcatsEnums.CreatureTemplateType.Yeek)           return 60;
            if (type == CreatureTemplate.Type.BigEel)                                       return 61;
            if (type == MoreSlugcats.MoreSlugcatsEnums.CreatureTemplateType.BigJelly)       return 62;
            if (type == CreatureTemplate.Type.TempleGuard)                                  return 63;
